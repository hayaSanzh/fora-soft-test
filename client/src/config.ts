...
/**
 * Значения подставляет Vite на этапе сборки. Обращение обёрнуто в приведение,
 * чтобы модуль компилировался и там, где типы `vite/client` не подключены.
 */
 ...
 * конфигурацию, поэтому дефолты и переопределения проверяются тестами напрямую,
 * без подмены глобальных объектов.
 */
export function readClientConfig(env: EnvSource = viteEnv) {
  return {
    // ── Сигналинг (TDD §4.1) ────────────────────────────────────────────────────
    /**
...
/** Конфигурация текущей сборки. Единственный экземпляр на приложение. */
export const config = readClientConfig();

export type ClientConfig = ReturnType<typeof readClientConfig>;

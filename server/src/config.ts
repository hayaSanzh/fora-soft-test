...
export function readServerConfig(env: EnvSource = process.env) {
  return {
    ...
    /**
     * Сигналинг: события `signal:*` за 10 с на сокет. Превышение — это уже не
     * человек, поэтому здесь сокет **отключается** (TDD §10.4).
     *
     * ★ Значение выведено из измерения, а не назначено (задача 14.6). Прежние
     * 100 событий были откалиброваны по Chromium и **отключали легитимных
     * пользователей Firefox**: на одно соединение 1:1 Chromium присылает 8
     * ICE-кандидатов, а Firefox — 99 (замер в E2E, хост с несколькими сетевыми
     * интерфейсами). Одного звонка вдвоём хватало, чтобы сервер разорвал
     * соединение через секунду после входа.
     *
     * Расчёт: worst case — 4 участника, то есть 3 соединения mesh, по ~100
     * кандидатов = ~300 событий на вход, плюс оффер/ответ и ренегоциации.
     * Порог 1000 даёт трёхкратный запас; при этом 100 событий в секунду
     * непрерывно — по-прежнему явная аномалия, а не поведение человека.
     */
    signalRateMax: num(env.SIGNAL_RATE_MAX, 1000),
